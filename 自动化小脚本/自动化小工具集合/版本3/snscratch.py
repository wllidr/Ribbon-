        ...
        with open(file, 'rb') as f:
            ...
            for line in f:
                ...
                if '#' in line.strip():
                    ...
                    if re.search('interface\s*MEth0/0/0', string) and re.search('ip\s*address', string):
                        methIp = re.search('ip\s*address\s*(\d*\.\d*\.\d*\.\d*)', string).groups()[0].strip()
                    if re.search('interface\s*LoopBack0', string) and re.search('ip\s*address', string):
                        loopback0Ip = re.search('ip\s*address\s*(\d*\.\d*\.\d*\.\d*)', string).groups()[0].strip()
                        # print(string)
                    if re.search('interface\s*LoopBack1', string) and re.search('ip\s*address', string):
                        loopback1Ip = re.search('ip\s*address\s*(\d*\.\d*\.\d*\.\d*)', string).groups()[0].strip()
                    if re.search('Configured\s*startup\s*system\s*software', string):
                        # print(string)
                        deviceVersion = re.findall('startup\s*system\s*software:\s*flash:/([\s\S]*?\.cc)', string)
                        deviceVersion = ' '.join(list(set(deviceVersion)))
                        devicePatch = re.findall('patch\s*package:\s*flash:/([\s\S]*?\.PAT)', string)
                        devicePatch = ' '.join(list(set(devicePatch)))
                    if re.search('ESN:([\s\S]*?)\n', string):
                        deviceSn = re.search('ESN:([\s\S]*?)\n', string).groups()[0].strip()
                    if re.search('Active\s*License', string):
                        licenseStatus = re.search('Expired\s*date\s*:([\s\S]*?)\n', string).groups()[0].strip()
                    string = ''
            if methIp.strip() == '':
                sortedInfos1.append({'deviceModel': deviceModel, 'sysname': sysname, 'methIp': methIp.strip(),
                                    'loopback0Ip': loopback0Ip,
                                    'loopback1Ip': loopback1Ip, 'deviceVersion': deviceVersion,
                                    'devicePatch': devicePatch,
                                    'deviceSn': deviceSn, 'licenseStatus': licenseStatus})
            else:
                sortedInfos.append({'deviceModel':deviceModel, 'sysname':sysname, 'methIp':methIp.strip(), 'loopback0Ip':loopback0Ip,
                                    'loopback1Ip':loopback1Ip, 'deviceVersion':deviceVersion, 'devicePatch':devicePatch,
                                    'deviceSn':deviceSn, 'licenseStatus':licenseStatus})
    # print(sortedInfos)

    sortedInfos = sorted(sortedInfos, key=lambda file: (int(file['methIp'].split('.')[-1]) + 1000 * int(file['methIp'].split('.')[-2]) + 1000 * 1000 * int(file['methIp'].split('.')[1])))
    # print(sortedInfos)
...
if __name__ == '__main__':
    SimpleSn(r'C:\Users\Ribbon\Desktop\20190311_1506')