...
def config(filePath):
    ...
    '''
    sheet_config = '配置参数'
    df2 = pd.read_excel(filePath, sheet_name = sheet_config).dropna(axis=0, how='all')
...
def templete(filePath, templeteRow):
    ...
    '''
    sheet_templete = '配置模板'
    df1 = pd.read_excel(filePath, sheet_name=sheet_templete, header=None).dropna(axis=0, how='all')
