    ...
    for device in devices:
        ...
        if '接入' in device['deviceRole'] :
            if str(device['stackNumber']) != '不堆叠':
                try:
                    acFile = [temp for temp in temps if device['deviceClass'][1:5] in temp and '堆叠' in temp]
                    if '2720' in device['deviceClass']:
                        if '52TP' in device['deviceClass']:
                            acFile = [file for file in acFile if '52TP' in file][0]
                        else:
                            acFile = [file for file in acFile if '52TP' not in file][0]
                    else:
                        acFile = acFile[0]
                except:
                ...
                try:
                    acFile = [temp for temp in temps if device['deviceClass'][1:5] in temp and '单机' in temp]
                    if '2720' in device['deviceClass']:
                        if '52TP' in device['deviceClass']:
                            acFile = [file for file in acFile if '52TP' in file][0]
                        else:
            ...
            i += 1
            # print(acFile)
            acFiles.append({'sysname':device['deviceName'], 'acFile': acFile.encode('gbk').decode('cp437'), 'manageIp':manageIp1 + '/' + str(int(mark)), 'stackNumber' : device['stackNumber'], 'deviceClass' :device['deviceClass']})
    # print(acFiles)
    return acFiles, swFile, fwFile

if __name__ == '__main__':
    devices = [{'deviceName': 'LIFE-SW-HN-ZZ-NYL-S2700-2F-04', 'deviceRole': '接入交换机', 'deviceClass': 'S2720-52TP-PWR-EI', 'stackNumber': '不堆叠', 'topDevice': '接入交换机'},
               {'deviceName': 'LIFE-SW-HN-ZZ-NYL-S2700-2F-04', 'deviceRole': '接入交换机','deviceClass': 'S2720-PWR-EI', 'stackNumber': 2, 'topDevice': '接入交换机'}]
    swModel = '5720HI'
