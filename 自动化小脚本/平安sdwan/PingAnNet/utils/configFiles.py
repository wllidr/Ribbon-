...
def fileSelect(bussniessClass, fwYN, devices, swModel, ipPlans):
    path = [file for file in os.listdir(os.path.dirname(os.path.dirname(__file__))) if '全场景标准配置脚本' in file][0]
    files = os.listdir(os.path.join(os.path.dirname(os.path.dirname(__file__)), path))
    ...
    if (bussniessClass[0]== 'D' or bussniessClass[0] == 'E') and fwYN == '有FW':
        ...
        fwFile = os.path.join(os.path.join(os.path.dirname(os.path.dirname(__file__)), path), fwFile[0])
    ...
    if bussniessClass[0] == 'A' or bussniessClass[0] == 'B' or bussniessClass[0] == 'C':
        ...
        swFile = os.path.join(os.path.join(os.path.dirname(os.path.dirname(__file__)), path), swFile[0])
        ...
        if '接入' in device['deviceRole'] :
            if str(device['stackNumber']) != '不堆叠':
                try:
                    ...
                    acFile = os.path.join(os.path.join(os.path.dirname(os.path.dirname(__file__)), path), acFile)
                except:
            ...
            i += 1
            acFiles.append({'sysname':device['deviceName'], 'acFile': acFile, 'manageIp':manageIp1 + '/' + str(int(ipPlan['mark'])), 'stackNumber' : device['stackNumber']})
...
if __name__ == '__main__':
    devices = [{'deviceName': 'LIFE-SW-HN-ZZ-NYL-S2700-2F-04', 'deviceRole': '核心交换机', 'deviceClass': 'S2700-26TP-EI-AC', 'stackNumber': '不堆叠', 'topDevice': '核心交换机'},
               {'deviceName': 'LIFE-SW-HN-ZZ-NYL-S2700-2F-05', 'deviceRole': '接入交换机', 'deviceClass': 'S2750-26TP-EI-AC', 'stackNumber': '堆叠', 'topDevice': '核心交换机'},
    ...
    for i in fileSelect('A类', '无FW', devices, swModel, ipPlans):
        print(i)